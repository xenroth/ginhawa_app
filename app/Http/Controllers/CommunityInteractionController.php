...
{
    public function comment(Request $request, Post $post) { abort_unless($request->user()->canPost(), 403); $data = $request->validate(['body' => ['required', 'string', 'max:5000']]); $post->comments()->create(['user_id' => $request->user()->id, 'body' => $data['body'], 'status' => 'approved']); return back()->with('success', 'Reply transmitted.'); }
