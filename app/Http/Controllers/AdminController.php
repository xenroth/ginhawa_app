...
{
    public function index() { return view('admin', ['users' => User::with('roles')->latest()->get(), 'posts' => Post::with('author')->latest()->get(), 'metrics' => ['users' => User::count(), 'pendingUsers' => User::where('status', 'pending')->count(), 'pendingPosts' => Post::where('status', 'pending')->count()], 'update' => session('update'), 'verifications' => VerificationDocument::with('user')->where('status', 'pending')->latest()->get()]); }
    ...
    public function settings() { return view('admin.settings', ['roles' => Role::latest()->get(), 'sectors' => Sector::orderBy('sort_order')->get(), 'directives' => Directive::orderBy('sort_order')->get(), 'settings' => SiteSetting::pluck('value', 'key')]); }
    public function verifications() { return view('admin.verifications', ['verifications' => VerificationDocument::with('user')->latest()->get()]); }
    public function updateSettings(Request $request) { $data = $request->validate(['seo_title' => ['nullable', 'string', 'max:160'], 'seo_description' => ['nullable', 'string', 'max:320'], 'seo_keywords' => ['nullable', 'string', 'max:320'], 'site_name' => ['required', 'string', 'max:100'], 'xrp_address' => ['nullable', 'string', 'max:100'], 'xrp_memo' => ['nullable', 'string', 'max:100'], 'favicon' => ['nullable', 'file', 'mimes:ico,png,svg', 'max:1024'], 'logo' => ['nullable', 'image', 'max:2048']]); foreach (['favicon' => 'site_favicon', 'logo' => 'site_logo'] as $input => $key) if ($request->hasFile($input)) SiteSetting::updateOrCreate(['key' => $key], ['value' => $request->file($input)->store('branding', 'public'), 'type' => 'file']); unset($data['favicon'], $data['logo']); foreach ($data as $key => $value) SiteSetting::updateOrCreate(['key' => $key], ['value' => $value, 'type' => 'text']); return back()->with('success', 'Global CMS settings updated.'); }
    ...
    public function storeSector(Request $request) { $data = $request->validate(['name' => ['required', 'string', 'max:80'], 'slug' => ['required', 'alpha_dash', 'unique:sectors,slug'], 'description' => ['nullable', 'string']]); Sector::create($data); return back()->with('success', 'Sector created.'); }
