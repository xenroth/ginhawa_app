...
class ForumController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('forum', ['posts' => $posts, 'sectors' => ['MANIFESTO', 'OPERATIVES', 'PRESERVATION', 'ARCHIVES', 'LOUNGE']]);
    }

    public function store(Request $request)
    {
        abort_unless($request->user()->canPost(), 403, 'Your registration is awaiting approval.');
        $data = $request->validate(['title' => ['required', 'string', 'max:180'], 'body' => ['required', 'string'], 'sector' => ['required', 'in:MANIFESTO,OPERATIVES,PRESERVATION,ARCHIVES,LOUNGE'], 'clearance' => ['required', 'in:public,member,enforcer'], 'tags' => ['nullable', 'string']]);
